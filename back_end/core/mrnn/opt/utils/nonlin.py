...
def get(name=None):
    if name == 'SOFTMAX' or name is None:
        return Softmax
    elif name == 'SIGMOID':
        return Sigmoid
    elif name == 'SQ_SIGMOID':
        return Squared_Sigmoid
    elif name == 'LINEAR':
        return Lin
    else:
        raise NotImplementedError("Not implemented nonlin function %s" % name)
...
class _Softmax_cls:
    ...
    def H_prod(self, X, P, M, H_damping=None):
        ...
        return P * (X - (P*X).sum(1).reshape(-1, 1)) * M

    def H_inv_prod(self, X, P, M):
        # actually this isn't correct, but it's approximately correct,
        # and precisely so if we remove the -(P*X).sum(1) term above.
        return X / P * M
...
class _Sigmoid_cls:
    ...
    def loss(self, XP, O, M):
        return ((O*g.log_1_plus_exp(-XP) + (1-O)*g.log_1_plus_exp(XP))*M).sum()
...
Squared_Sigmoid = _Squared_Sigmoid_cls(1)
...
class _True_Sigmoid_cls:
    ...
    def loss(self, XP, O, M):
        def H(X):
            from ersatz.mrnn.gnumpy import log
            return -(X*log(X+1e-10) + (1-X)*log(1-X+1e-10))
        return ((O*g.log_1_plus_exp(-XP) + (1-O)*g.log_1_plus_exp(XP) - H(O))*M).sum()
...
"""
    ...
    def H_prod(self, X, P, M, H_damping=None):
        ...
        ANS = 0*X
        for (f, (ir0,ir1), (or0,or1), h, w) in zip(self.fns, self.input_ranges, self.output_ranges, H_damping, self.weights):
    ...
    def loss(self, XP, O, M):
        ans = 0
        for (i, (f, (ir0,ir1), (or0,or1), w)) in enumerate(zip(self.fns, self.input_ranges, self.output_ranges, self.weights)):
    ...
    def grad(self, XP, O, M):
        ANS = XP*0
        for (f, (ir0,ir1), (or0,or1), w) in zip(self.fns, self.input_ranges, self.output_ranges, self.weights):
...
def _rect(x): return g.log_1_plus_exp(x)
def _rect_prime_y(y): 
