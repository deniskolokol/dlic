...
try:
    from data_management.dmworker.formats import zip_with_mrnn_csv, zip_with_batches
except ImportError:
    pass
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for df in orm['data_management.DataFile'].objects.all():
            if df.state != 5:
                if df.meta:
                    if zip_with_mrnn_csv in df.meta['formats']:
                        df.mrnn_format = True
                        print df.key, 'mrnn_format=True'
                    if zip_with_batches in df.meta['formats']:
                        df.batches_format = True
                        print df.key, 'batches_format=True'
                    df.save()
