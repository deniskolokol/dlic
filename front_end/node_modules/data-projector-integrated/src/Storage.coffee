...
class Storage extends Subject
   ...
   requestData : ->
      ...
      @requestDatafile("data.json")


   # Get the name of the data file to use in visualization.
   requestDatafile : (filename) -> @onDatafile filename
   ...
   # Manually send data via javascript call.
   injectCSV: (csv) =>
      p = csv.map (item) -> {x:item[0], y:item[1], z:item[2], cid:item[3]}
      @onJSON({points: p})
   ...
   processPoint : (nodeName, nodeData) =>
      ...
      unless nodeData.cid in @clusterIDs
         @cidIndicies[nodeData.cid] = @clusterIDs.length
   ...
   # Get the index for this cid
   getCidIndex : (cid) -> return @cidIndicies[cid]
