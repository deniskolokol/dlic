...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for model in orm['job.LearnModel'].objects.filter(
            model_name__in=['MLP_SIGMOID', 'MLP_MAXOUT',
                            'MLP_MAXOUT_CONV', 'MLP_RECTIFIED']
        ):
            params = model.model_params
            if 'learning_rate' in params and 'start' in params['learning_rate']:
                try:
                    del params['learning_rate']['start']
                    del params['learning_rate']['stop']
                except KeyError:
                    pass
                params['learning_rate']['decay_factor'] = 1.00004
