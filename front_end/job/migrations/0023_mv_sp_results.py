...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for ensemble in orm['job.TrainEnsemble'].objects.all():
            if ensemble.sp_results:
                sp_results = ensemble.sp_results.strip().split('\n')
                for model in ensemble.learn_models.filter(state='FINISHED').order_by('pk'):
                    if sp_results:
                        model.sp_results = sp_results.pop(0)
                        model.save()
                    else:
                        break
    ...
    def backwards(self, orm):
        ...
        for ensemble in orm['job.TrainEnsemble'].objects.all():
            sp_results = ensemble.learn_models.all().order_by('pk'
                    ).values_list('sp_results', flat=True)
            sp_results = [x for x in sp_results if x]
            if sp_results:
                sp_results = '\n'.join(sp_results)
                ensemble.sp_results = sp_results
                ensemble.save()
