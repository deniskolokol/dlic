...
class Migration(DataMigration):
    ...
    def update_acc_matrix(self, matrix):
        new = []
        for row in matrix:
            new.append([x / 100. for x in row])
        return new

    def update_mrnn_stat(self, model):
        for stat in model.stats.exclude(discarded=True):
            stat.train_accuracy /= 100.
            stat.test_accuracy /= 100.
            data = stat.data
            data['train_accuracy'] /= 100.
            data['test_accuracy'] /= 100.
            data['test_last_10_steps_acc'] /= 100.
            data['train_last_10_steps_acc'] /= 100.
            data['test_accuracy_matrix'] = self.update_acc_matrix(data['test_accuracy_matrix'])
            data['train_accuracy_matrix'] = self.update_acc_matrix(data['train_accuracy_matrix'])
            stat.save()

    def update_mlp_stat(self, stat):
        stat.train_accuracy /= 100.
        stat.test_accuracy /= 100.
        data = stat.data
        data['train_accuracy'] /= 100.
        data['test_accuracy'] /= 100.
        for val in data['train_outputs']:
            val[1] /= 100.
            val[2] /= 100.
        stat.save()

    def forwards(self, orm):
        for model in orm['job.LearnModel'].objects.exclude(state='DELETED', model_name__in=['AUTOENCODER', 'CONV']).prefetch_related('stats'):
            if model.model_name == 'MRNN':
                vals = model.stats.exclude(discarded=True).values('train_accuracy', 'test_accuracy')
                if vals:
                    train_max = max(x['train_accuracy'] for x in vals)
                    test_max = max(x['test_accuracy'] for x in vals)
                    if train_max > 1 or test_max > 1:
                        self.update_mrnn_stat(model)
            elif model.model_name.startswith('MLP_'):
                # this models have maximum one stat
                stats = model.stats.exclude(discarded=True)
                if stats:
                    stat = stats[0]
                    if stat.train_accuracy > 1 or stat.test_accuracy > 1:
                        self.update_mlp_stat(stat)
