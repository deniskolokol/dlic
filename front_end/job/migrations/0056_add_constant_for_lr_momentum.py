...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for model in orm['job.LearnModel'].objects.filter(
            model_name__in=('MLP_MAXOUT', 'MLP_SIGMOID', 'MLP_RECTIFIED')):
            if (isinstance(model.model_params, dict) and
                'learning_rate' in model.model_params and
                isinstance(model.model_params['learning_rate'], dict)):

                model.model_params['learning_rate']['constant'] = False
                model.save()
    ...
    def backwards(self, orm):
        ...
        for model in orm['job.LearnModel'].objects.filter(
            ...
            if (isinstance(model.model_params, dict) and
                'learning_rate' in model.model_params and
                isinstance(model.model_params['learning_rate'], dict) and
                'constant' in model.model_params['learning_rate']):

                del model.model_params['learning_rate']['constant']
                model.save()
