...
def run(ids_chunk, k, orm):
    for modeldata in orm['job.ModelData'].objects.filter(statistic_id__in=ids_chunk).select_related('statistic'):
        if modeldata.data:
            prefix = ''.join([choice(string.digits + string.letters) for i in range(0, 8)])
            k.key = '/modeldata/'  + str(modeldata.statistic.model_id) + '/' + \
                    str(modeldata.statistic_id) + '_' + prefix + '.json'
            data = json.dumps(modeldata.data)
            bts = k.set_contents_from_string(data)
            if bts != len(data):
                raise Exception('Uploaded data size not equal to the real size')
            statistic = modeldata.statistic
            statistic.s3_data = k.key
            statistic.save()
...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        pass
        # NOTE: it was used for production server
        #conn = S3Connection(settings.AWS_ACCESS_KEY, settings.AWS_SECRET_KEY)
        #bucket_name = settings.S3_BUCKET
        #bucket = conn.get_bucket(bucket_name)
        #k = Key(bucket)
        #ids = orm['job.ModelData'].objects.values_list('statistic_id', flat=True)
