...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        for ens in orm['job.TrainEnsemble'].objects.all():
            train = ens.file_id_id
            test = ens.test_dataset_id
            valid = ens.valid_dataset_id
            split = ens.data_split
            ens.old_data = [train, test, valid, split]
            ens.save()

    def backwards(self, orm):
        for ens in orm['job.TrainEnsemble'].objects.all():
            train, test, valid, split = ens.old_data
            ens.file_id_id = train
            ens.test_dataset_id = test
            ens.valid_dataset_id = valid
            ens.data_split = split
            ens.save()
    ...
    models = {
        ...
        u'job.trainensemble': {
            ...
            'error': ('django.db.models.fields.TextField', [], {'null': 'True', 'blank': 'True'}),
            'file_id': ('django.db.models.fields.related.ForeignKey', [], {'blank': 'True', 'related_name': "'train_ensembles'", 'null': 'True', 'to': u"orm['data_management.DataFile']"}),
            u'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
