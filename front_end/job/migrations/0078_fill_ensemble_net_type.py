...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for ensemble in orm['job.TrainEnsemble'].objects.all():
            if ensemble.data_type == 'TIMESERIES':
                ensemble.net_type = 'RNN'
            elif ensemble.data_type == 'IMAGES':
                ensemble.net_type = 'DEEPNET'
            elif ensemble.data_type == 'GENERAL':
                lms = orm['job.LearnModel'].objects \
                    .filter(ensemble=ensemble.pk) \
                    .values_list('model_name', flat=True)
                if set(lms) == {'AUTOENCODER'}:
                    ensemble.net_type = 'AUTOENCODER'
                else:
                    ensemble.net_type = 'DEEPNET'
            else:
                lms = orm['job.LearnModel'].objects \
                ...
                if set(lms) == {'AUTOENCODER'}:
                    ...
                    ensemble.net_type = 'MRNN'
                elif len(set(lms)) > 1:
                    raise Exception('More than one model type in ensemble')
                else:
                    ensemble.net_type = 'DEEPNET'
            ensemble.save()
    ...
    models = {
        ...
        u'job.trainensemble': {
            ...
            'error': ('django.db.models.fields.TextField', [], {'null': 'True', 'blank': 'True'}),
            u'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
            'net_type': ('django.db.models.fields.CharField', [], {'max_length': '20', 'null': 'True'}),
