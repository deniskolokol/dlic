...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for ensemble in orm['job.TrainEnsemble'].objects.select_for_update().all():
            states = set(ensemble.learn_models.exclude(state='DELETED').values_list('state', flat=True))
            state = 'in queue'
            if ensemble.deleted:
                state = 'deleted'
            elif 'ERROR' in states or ensemble.error:
                state = 'error'
            elif states == set(['FINISHED']):
                state = 'finished'
            elif ensemble.canceled:
                state = 'stopped'
            elif 'TRAIN' in states:
                state = 'training'
            elif 'QUEUE' in states:
                pass
            elif 'NEW' in  states:
                state = 'new'
            elif not states:
